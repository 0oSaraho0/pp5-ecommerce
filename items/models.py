...
class Item(models.Model):
    ...
    sku = models.UUIDField(
        primary_key=True, default=uuid.uuid4, editable=False, max_length=2254)
    name = models.CharField(max_length=254)
    clothes_type = models.ForeignKey(
        'ClothesType', null=True, blank=True, on_delete=models.SET_NULL)
    men_women_children = models.CharField(
        max_length=200, choices=MEN_WOMEN_CHILDREN, default='na')
    size = models.ForeignKey(
        'Size', null=True, blank=True, on_delete=models.SET_NULL)
    kids_age = models.ForeignKey(
        'KidsAge', null=True, blank=True, on_delete=models.SET_NULL)
    colour = models.ForeignKey(
        'Colour', null=True, blank=True, on_delete=models.SET_NULL)
    brand_or_author = models.ForeignKey(
        'BrandOrAuthor', null=True, blank=True, on_delete=models.SET_NULL)
